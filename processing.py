    ...
    with open("people_stat.txt", 'w') as output_file:
        for person in people:
            ...
            if person.kkks_count.__len__() > 0:
                output_file.write(str(np.sum(person.kkks_count))    + ';')
                output_file.write(str(np.max(person.kkks_count))    + ';')
                output_file.write(str(np.min(person.kkks_count))    + ';')
                output_file.write(str(np.median(person.kkks_count)) + ';')
